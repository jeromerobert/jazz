...
public class ZText extends ZVisualComponent {
    ...
    */
	...
	caretLine = (int)((pt.getY()-desc)/height) + 1;
	...
	for (int ch=0; ch <= textLine.length(); ch++) {
	    ...
	    if ((javaVersion.equals("1.2")) ||
		(javaVersion.equals("1.2.1"))) {
		if ((substr != null) && (substr.length() > 0) &&
		    (substr.charAt(substr.length()-1) == ' ')) {
		    strWid += font.getStringBounds("r", prevFRC).getWidth();
		}
	    }
	...
	* @param <code>e</code> keyboard event object.
    ...
    /**
       ...
       * @param <code>renderContext</code> Contains information about
       current render.
    */
	...
	if (!lines.isEmpty()) {
	    ...
	    if (((font.getSize()*renderContext.getCompositeMagnification()) < greekThreshold)
		 && (renderContext.getGraphics2D().getFontRenderContext().isAntiAliased() == false)) {
	...
	for (Iterator i = lines.iterator() ; i.hasNext() ; ) {
	    ...
	    lm = font.getLineMetrics(line, frc);
				//lm.getHeight = ascent+descent+leading
	    g2.drawString(line, 0, lm.getAscent() + (lineNum * lm.getHeight()));
	    lineNum++;
 	}
	...
	if (editable) {
		...
		if ((javaVersion.equals("1.2")) ||
			...
			(textLine.charAt(caretPos-1) == ' ')) {
			// all trailing spaces are zero, gotta count them:
			int numberOfSpaces = 0;
			for (int j = caretPos-1; j > -1; j--) {
			    if (textLine.charAt(j) != ' ') break;
			    numberOfSpaces++;
			}
			caretX += font.getStringBounds("r", frc).getWidth()
			    * numberOfSpaces;
		    }
		}
	    }
	...
	for (int loop=0; loop<2; loop++) {
		...
		for (Iterator i = lines.iterator() ; i.hasNext() ; ) {
			...
			if ((line.length() > 0) &&
			    ...
			    lineWidth += font.getStringBounds("r", frc).getWidth() * numberOfSpaces;
			}
		    }
		...
		bounds = font.getStringBounds("r", frc);
