...
/** 
 * ZNode is the provides basic containment hierarchies.  It maintains a bag of
...
public class ZNode implements ZScenegraphObject, ZSerializable {
    ...
    static public final boolean hidden_DEFAULT = false;
    ...
    /**
     * The visual content of this node.  Can never be null (should be the dummy conponent rather
     * than null).  Often abbrevieated to VC.
    */
    ...
    */
     ...
     */
    protected boolean hidden = hidden_DEFAULT;
    ...
    /** 
     * Raises this child to the front of the node's children list.  This has the effect
     * of making this <i>child</i> appear above all other children.
     * 
     * @param child 
     */
    public void raise(ZNode child) {
	boolean exists = children.removeElement(child);
	if (exists) {
    ...
    /** 
     * Lowers this child to the back of the node's children list.  This has the effect
     * of making this <i>child</i> appear below all other children.
     * 
     * @param child 
     */
    ...
    /** 
     * Advances this child one position to forwards in node's children list.  Children in
     * front of <i>child</i> in the list will be display in front of <i>child</i> and children
     * behind <i>child</i> in the list will be display below <i>child</i>.
     * 
     * @param child 
     */
    public void raise() {
    ...
    /** 
     * Moves this child to an exact position in  the node's children list.  Children in
     * front of <i>child</i> in the list will be display in front of <i>child</i> and children
     * behind <i>child</i> in the list will be display below <i>child</i>.
     * 
     * @param child 
    ...
    /** 
     * Demotes this child one position to backwards in the node's children list.  Children in
     * front of <i>child</i> in the list will be display in front of <i>child</i> and children
     * behind <i>child</i> in the list will be display below <i>child</i>.
     * 
     * @param child 
    ...
    /**
     * Get the value of hidden.  Nodes with the hidden flag set to true are not painted
     * or displayed.  Note that hidden nodes do not contribute do the bounds of their
     * parents.
     * Same as isHidden().
    ...
    /**
     * Set the value of hidden.  Nodes with the hidden flag set to true are not painted
     * or displayed.  Note that hidden nodes do not contribute do the bounds of their
     * parents.
     * @param h Value of hidden.
     */
     ...
     * Nothing is painted if the node is hidden or is magnified more
	...
	float           currentMag = renderContext.getCameraMagnification();
				...
				// Don't paint nodes that are hidden, too big or too small
	if (hidden ||
	    (currentMag < minMag) ||
    ...
    public ZNode pick(Rectangle2D rect, float mag) {
				// If node is hidden, don't pick it
	if (hidden ||
	    (mag < minMag) ||
	    ((maxMag >= 0) && (mag > maxMag))) {
	    return null;
	...
	ZNode child;
				...
				// Go no further if this node is hidden or not findable
	if (hidden || (!getVisualComponent().isFindable())) {
   ...
   /**
     ...
     * application code.  Note that if a node is hidden, it does not contribute
    ...
    */
	...
	if (hidden != hidden_DEFAULT) {
	    out.writeState("boolean", "hidden", hidden);
	}
	...
	if (properties != null) {
	    out.writeState("Vector", "properties", properties);
	}
    }
	...
	} else if (fieldName.compareTo("hidden") == 0) {
	    hidden = ((Boolean)fieldValue).booleanValue();
