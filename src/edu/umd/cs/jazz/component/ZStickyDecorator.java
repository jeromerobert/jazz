...
public class ZStickyDecorator extends ZConstraintDecorator {
    protected ZNodeListener nodeListener = null;
    protected boolean inTransformEventListener = false;
    ...
    /**
     * Override method so that we can trap transform changes in the node above this sticky decorator
     * to make sticky objects transform like other nodes.
     */
    ...
    public void setParent(ZScenegraphObject aParent) {
				// Clear node transform listener if there was one
	ZNode node = findNode();
	if ((node != null) && (nodeListener != null)) {
	...
	ZNode node = findNode();
	if (node != null) {
	    nodeListener = new ZNodeAdapter() {
		public void nodeTransformed(ZNodeEvent e) {
		    if (!inTransformEventListener) {
			inTransformEventListener = true;
			...
			{
			    /*
			    AffineTransform delta = e.getOrigTransform().createInverse();
			    delta.concatenate(e.getNode().getTransform().getAffineTransform());
			    AffineTransform t = e.getOrigTransform();
			    t.concatenate(camera.getViewTransform().getAffineTransform());
			    t.concatenate(delta);
			    e.getNode().setTransform(new ZTransform(t));
			    */

			    AffineTransform delta = e.getOrigTransform().createInverse();
			    delta.concatenate(e.getNode().getTransform().getAffineTransform());
			    System.out.println("Delta = " + delta);
			    System.out.println("View  = " + camera.getViewTransform().getAffineTransform());
			}
			catch (NoninvertibleTransformException exc) {
			    System.out.println("Can't compute inverse transform while updating sticky node transform");
			}
			inTransformEventListener = false;
		    }
		}
	    };
    ...
    /**
     * We need to stop trapping transform changes in the node above this sticky decorator
     * since it is being removed from a node.
     */
    public void remove() {
	ZNode node = findNode();
				// Clear node transform listener if there was one
