...
public class ZCamera extends ZNode {
    ...
    /**
     * Transform the point through the camera.  In the typical case
     * where this is a top-level camera, and the point represents
     * a coordinate in screen coordinates, this will transform the
     * point to Jazz global coordinates.  The input point is modified
     * by this method.
     * @param pt The point to be transformed
     * @see #sceneToCamera(Point2D)
     */
    public void cameraToScene(Point2D pt) {
	getInverseViewTransform().transform(pt, pt);
    }

    /**
     * Transform the rectangle through the camera.  In the typical case
     * where this is a top-level camera, and the rectangle represents
     * coordinates in screen coordinates, this will transform the
     * rectangle to Jazz global coordinates.  The input rectangle is modified
     * by this method.
     * @param rect The rectangle to be transformed
     * @see #sceneToCamera(Rectangle2D)
     */
    public void cameraToScene(Rectangle2D rect) {
	getInverseViewTransform().transform(rect, rect);
    }

    /**
     * Transform the point backwards through the camera (from the camera's
     * viewpoint).  In the typical case where this is a top-level camera, 
     * and the point represents a coordinate in Jazz global coordinates, 
     * this will transform to screen coordinates.  The input point is modified
     * by this method.
     * @param pt The point to be transformed
     * @see #cameraToScene(Point2D)
     */
    public void sceneToCamera(Point2D pt) {
	getViewTransform().transform(pt, pt);
    }

    /**
     * Transform the rectangle backwards through the camera (from the camera's
     * viewpoint).  In the typical case where this is a top-level camera, 
     * and the rectangle represents coordinates in Jazz global coordinates, 
     * this will transform to screen coordinates.  The input rectangle is modified
     * by this method.
     * @param rect The rectangle to be transformed
     * @see #cameraToScene(Rectangle2D)
     */
    public void sceneToCamera(Rectangle2D rect) {
	getViewTransform().transform(rect, rect);
    }
