...
public class ZTransform implements ZSerializable, Cloneable {
	...
	AffineTransform origTransform = new AffineTransform(transform);
	if (parent != null) {
	    parent.damage();
	}
	...
	if (parent != null) {
	    ...
	    parent.damage();
	    parent.transformChanged(origTransform);
	}
    }
   ...
   /**
	...
	if (parent != null) {
	    ...
	    Rectangle2D srcBounds = node.getGlobalBounds();
     ...
     * @param millis Number of milliseconds over which to perform the animation
     * @param surface The surface to updated during animation.
     * @see #animate(AffineTransform, int, ZSurface, ZLerp)
     */
    public void animate(AffineTransform tx, int millis, ZSurface surface) {
	animate(tx, millis, surface, new ZSISOLerp());
    }
     ...
     * @see #animate(AffineTransform, int, ZSurface)
     */
    public void animate(AffineTransform tx, int millis, ZSurface surface, ZLerp lerpTimeFunction) {
	...
	if (parent != null) {
	    ...
	    node.damage();
	    node.transformChanged(origTransform);
     ...
     * @param millis Number of milliseconds over which to perform the animation
     * @param surface The surface to updated during animation.
     * @see #animate(ZNode[], AffineTransform[], int, ZSurface, ZLerp)
     */
     ...
     * @param lerpTimeFunction The function that determines how the timing of the animation should be calculated
     */
    static public void animate(ZNode[] nodes, AffineTransform[] txs, int millis, ZSurface surface, ZLerp lerpTimeFunction) {
	int i;
	...
	AffineTransform[] origTransforms = new AffineTransform[len];
	ZNode node;
	...
	for (i=0; i<len; i++) {
	    ...
	    origTransforms[i] = new AffineTransform(nodes[i].getTransform().getAffineTransform());
	}
	...
	for (i=0; i<len; i++) {
	    ...
	    nodes[i].transformChanged(origTransforms[i]);
	}
 ...
 */
class ZSISOLerp implements ZLerp {
    public float lerpTime(float t) {
	float siso, t1, t2, l;
	
	t1 = t * t;
	t2 = 1 - (1 - t) * (1 - t);
	l = ZUtil.lerp(t, t1, t2);
	siso = ZUtil.lerp(l, t1, t2);
